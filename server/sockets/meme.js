  ...
  socket.on('meme:vote', async (data, callback) => {
    try {
      ...
      if (isSuper) {
        ...
        if (voteType === 'up' && participant?.superVoteUsedThisRound) {
          return callback?.({ success: false, message: 'Super vote déjà utilisé cette manche' });
        }
        if (voteType === 'down' && participant?.superDownvoteUsedThisRound) {
          return callback?.({ success: false, message: 'Super downvote déjà utilisé cette manche' });
        }
      }
