/**
 * Mot Imposteur (type Undercover) — état EN MÉMOIRE par lobby.
 * Phase 1 : lobby + config (mode, nb imposteurs, Mr White, vol de victoire, thème, paires custom)
 * + attribution des rôles/mots au démarrage + écran "ton mot" (chacun ne voit que le sien).
 * (Manches d'indices + vote + fin : Phases 2/3.)
 */
  ...
  return {
    ...
    players: l.order.map((id) => {
      ...
      return {
        ...
        role: reveal ? (p.role || null) : null,     // 'civil' | 'impostor' | 'mrwhite'
        word: reveal ? (p.word ?? null) : null,     // masqué pour les autres pendant le jeu
      };
    }),
  };
}
async function broadcast(io, l) {
  try {
    const sockets = await io.in(room(l.code)).fetchSockets();
    for (const s of sockets) s.emit('impostor:lobbyState', publicLobby(l, l.socketToPlayer?.[s.id] || null));
...
function register(socket, io) {
  ...
  socket.on('impostor:createLobby', async (data, cb) => {
    try {
      ...
      for (const [c, l] of impostorLobbies) { if (l.hostId === odId) { socket.to(room(c)).emit('impostor:gameEnded', { code: c }); impostorLobbies.delete(c); } }
      let themes = [];
      try { themes = await db.getImpostorThemes(); } catch (e) {}
      const code = generateCode();
      const lobby = {
        code, hostId: odId, status: 'waiting', phase: null,
        mode: 'quick', impostorCount: 1, mrWhite: false, stealWin: false, theme: null,
        themes, customPairs: [],
  ...
  socket.on('impostor:joinLobby', (data, cb) => {
    try {
      ...
      if (!odId) return cb?.({ success: false, message: 'Utilisateur invalide' });
      const host = isHost(l, odId);
      const existing = l.players[odId];
      let spectator = false;
      if (!host) {
        if (l.status === 'waiting') { if (!existing) { l.players[odId] = { odId, pseudo, avatar, avatarUrl: avatarUrl || null }; l.order.push(odId); } }
  ...
  socket.on('impostor:addCustomPair', (data, cb) => {
    ...
    l.customPairs.push({ civil, impostor });
    broadcast(io, l); cb?.({ success: true });
  });
  socket.on('impostor:removeCustomPair', (data, cb) => {
    const l = impostorLobbies.get(String(data?.code || '').trim());
    if (!isHost(l, data?.odId)) return cb?.({ success: false, message: "Réservé à l'hôte" });
    if (Number.isInteger(data.index)) l.customPairs.splice(data.index, 1);
    broadcast(io, l); cb?.({ success: true });
  });
  ...
  socket.on('impostor:leaveLobby', (data, cb) => {
    ...
    if (l) {
      ...
      if (l.players[odId] && l.status === 'waiting') {
        ...
        if (odId === l.hostId) { if (l.order.length) l.hostId = l.order[0]; else { impostorLobbies.delete(l.code); socket.leave(room(l.code)); return cb?.({ success: true }); } }
        broadcast(io, l);
      }
  ...
  socket.on('impostor:stopGame', (data, cb) => {
    ...
    impostorLobbies.delete(l.code); cb?.({ success: true });
  });

  // Démarrage : tire une paire, attribue les rôles/mots, passe en "reveal"
  socket.on('impostor:startGame', async (data, cb) => {
    try {
      ...
      if (n - extra < 2) return cb?.({ success: false, message: 'Trop d\'imposteurs pour le nombre de joueurs (il faut au moins 2 civils)' });

      // Pool = banque (filtrée par thème) + paires custom
      let bankPairs = [];
      try { const p = await db.getRandomImpostorPair(l.theme); if (p) bankPairs = [p]; } catch (e) {}
      const pool = [...(l.customPairs || []), ...bankPairs];
      if (!pool.length) return cb?.({ success: false, message: 'Aucune paire de mots disponible (ajoutez-en ou choisissez un autre thème)' });
