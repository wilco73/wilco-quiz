...
async function getLobbyWithTimer(lobby) {
...
async function broadcastGlobalState() {
  const lobbies = await await db.getAllLobbies().map(l => getLobbyWithTimer(l));
  const teams = await await db.getAllTeams();
  const participants = await await db.getAllParticipants().map(p => ({ ...p, password: '********' }));
  const quizzes = await await db.getAllQuizzes();
  const questions = await await db.getAllQuestions();
...
io.on('connection', (socket) => {
  ...
  socket.emit('global:state', {
    lobbies: await db.getAllLobbies().map(l => getLobbyWithTimer(l)),
    teams: await db.getAllTeams(),
    participants: await db.getAllParticipants().map(p => ({ ...p, password: '********' })),
  ...
  socket.on('auth:login', async (data, callback) => {
    ...
    const newParticipant = await await db.createParticipant(odId, pseudo, password, null);
  ...
  socket.on('lobby:create', async (data, callback) => {
    ...
    const lobby = await await db.createLobby(quizId, shuffle);
  ...
  socket.on('admin:resetScores', async (callback) => {
  ...
  socket.on('drawingLobby:join', async (data, callback) => {
    ...
    const lobby = await await db.joinDrawingLobby(lobbyId, odId, teamName);
  ...
  socket.on('drawingLobby:leave', async (data, callback) => {
    ...
    const lobby = await await db.leaveDrawingLobby(lobbyId, odId);
  ...
  socket.on('drawingLobby:addCustomWord', async (data, callback) => {
    ...
    // Sauvegarder
    const updatedLobby = await await db.updateDrawingLobbyCustomWords(lobbyId, customWords);
...
app.get('/api/config', (req, res) => (async () => {
  res.json({
    serverTime: Date.now(),
    version: '3.0.0',
...
app.post('/api/admin-login', (req, res) => (async () => {
...
// Teams
app.get('/api/teams', (req, res) => (async () => {
  res.json(await db.getAllTeams());
});

app.post('/api/teams/create', (req, res) => (async () => {
  ...
  const team = await await db.createTeam(normalizedName);
...
app.put('/api/teams/:id', (req, res) => (async () => {
  const { id } = req.params;
...
app.post('/api/delete-team', (req, res) => (async () => {
  ...
  const participants = await await db.getAllParticipants();
...
// Participants
app.get('/api/participants', (req, res) => (async () => {
  const participants = await await db.getAllParticipants().map(p => ({
    ...p,
    password: '********'
  }));
...
app.post('/api/participants/create', (req, res) => (async () => {
  ...
  const odId = `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  const participant = await await db.createParticipant(odId, pseudo.trim(), password, teamId);
  
  broadcastGlobalState();
  res.json({ success: true, participant });
});

app.put('/api/participants/:id', (req, res) => (async () => {
  const { id } = req.params;
...
app.delete('/api/participants/:id', (req, res) => (async () => {
  const { id } = req.params;
...
app.put('/api/participants/:id/team', (req, res) => (async () => {
  const { id } = req.params;
  const { teamName } = req.body;
...
app.put('/api/participants/:id/password', (req, res) => (async () => {
  const { id } = req.params;
...
app.put('/api/participants/:id/avatar', (req, res) => (async () => {
  const { id } = req.params;
  ...
  const updatedParticipant = await await db.updateParticipantAvatar(id, avatar);
  broadcastGlobalState();
...
app.post('/api/update-participant', (req, res) => (async () => {
...
app.post('/api/change-password', (req, res) => (async () => {
...
// Quizzes
app.get('/api/quizzes', (req, res) => (async () => {
  res.json(await db.getAllQuizzes());
});

app.post('/api/quizzes', (req, res) => (async () => {
  const quiz = req.body;
  const created = await await db.createQuiz(quiz);
  broadcastGlobalState();
  res.json({ success: true, quiz: created });
});

app.put('/api/quizzes/:id', (req, res) => (async () => {
  const { id } = req.params;
  const quiz = req.body;
  const updated = await await db.updateQuiz(id, quiz);
  broadcastGlobalState();
  res.json({ success: true, quiz: updated });
});

app.delete('/api/quizzes/:id', (req, res) => (async () => {
  const { id } = req.params;
...
// Questions
app.get('/api/questions', (req, res) => (async () => {
  res.json(await db.getAllQuestions());
});

app.post('/api/questions', (req, res) => (async () => {
...
app.post('/api/questions/add', (req, res) => (async () => {
  const question = req.body;
  const created = await await db.createQuestion(question);
  broadcastGlobalState();
  res.json({ success: true, question: created });
});

app.post('/api/questions/merge', (req, res) => (async () => {
  ...
  const stats = await await db.mergeQuestions(questions, mode || 'update');
  broadcastGlobalState();
  res.json({ success: true, stats });
});

app.put('/api/questions/:id', (req, res) => (async () => {
  const { id } = req.params;
  const question = req.body;
  const updated = await await db.updateQuestion(id, question);
  broadcastGlobalState();
  res.json({ success: true, question: updated });
});

app.delete('/api/questions/:id', (req, res) => (async () => {
  const { id } = req.params;
...
app.get('/api/lobbies', (req, res) => (async () => {
  res.json(await db.getAllLobbies().map(l => getLobbyWithTimer(l)));
});

// Archiver/Desarchiver un lobby
app.put('/api/lobbies/:id/archive', (req, res) => (async () => {
  const { id } = req.params;
  ...
  const updatedLobby = await await db.archiveLobby(id, archived);
  broadcastGlobalState();
...
app.get('/api/drawing-words', (req, res) => (async () => {
  res.json(await db.getAllDrawingWords());
});

app.post('/api/drawing-words', (req, res) => (async () => {
  const word = await await db.createDrawingWord(req.body);
  broadcastGlobalState();
  res.json({ success: true, word });
});

app.put('/api/drawing-words/:id', (req, res) => (async () => {
  const word = await await db.updateDrawingWord(req.params.id, req.body);
  broadcastGlobalState();
  res.json({ success: true, word });
});

app.delete('/api/drawing-words/:id', (req, res) => (async () => {
...
app.post('/api/drawing-words/import', (req, res) => (async () => {
  ...
  const results = await await db.mergeDrawingWords(words, mode);
  ...
  res.json({ 
    ...
    total: await db.getAllDrawingWords().length 
  });
});

// ==================== DRAWING REFERENCES API ====================

app.get('/api/drawing-references', (req, res) => (async () => {
  res.json(await db.getAllDrawingReferences());
});

app.post('/api/drawing-references', (req, res) => (async () => {
  const ref = await await db.createDrawingReference(req.body);
  broadcastGlobalState();
  res.json({ success: true, reference: ref });
});

app.put('/api/drawing-references/:id', (req, res) => (async () => {
  const ref = await await db.updateDrawingReference(req.params.id, req.body);
  broadcastGlobalState();
  res.json({ success: true, reference: ref });
});

app.delete('/api/drawing-references/:id', (req, res) => (async () => {
...
app.post('/api/drawing-references/import', (req, res) => (async () => {
  ...
  // Utiliser la fonction de merge optimisée
  const results = await await db.mergeDrawingReferences(references, mode);
  ...
  res.json({ 
    ...
    skipped: results.skipped,
    errors: results.errors,
    total: await db.getAllDrawingReferences().length 
  });
});

// ==================== DRAWING GAMES API ====================

app.get('/api/drawing-games', (req, res) => (async () => {
  res.json(db.getAllDrawingGames());
});

app.post('/api/drawing-games', (req, res) => (async () => {
  const game = db.createDrawingGame(req.body);
  broadcastGlobalState();
  res.json({ success: true, game });
});

app.put('/api/drawing-games/:id', (req, res) => (async () => {
  const game = db.updateDrawingGame(req.params.id, req.body);
  broadcastGlobalState();
  res.json({ success: true, game });
});

app.delete('/api/drawing-games/:id', (req, res) => (async () => {
...
app.get('/api/drawing-lobbies', (req, res) => (async () => {
  res.json(await db.getAllDrawingLobbies());
});

app.get('/api/drawing-lobbies/:id', (req, res) => (async () => {
...
app.post('/api/drawing-lobbies', (req, res) => (async () => {
  const lobby = await db.createDrawingLobby(req.body);
  broadcastGlobalState();
  res.json({ success: true, lobby });
});

app.delete('/api/drawing-lobbies/:id', (req, res) => (async () => {
...
app.post('/api/drawing-lobbies/:id/archive', (req, res) => (async () => {
...
app.post('/api/drawing-lobbies/:id/custom-words', (req, res) => (async () => {
  const { customWords } = req.body;
  const lobby = await await db.updateDrawingLobbyCustomWords(req.params.id, customWords || []);
  if (lobby) {
    // Notifier tous les participants
...
app.get('/api/drawing-lobbies/:id/results', (req, res) => (async () => {
